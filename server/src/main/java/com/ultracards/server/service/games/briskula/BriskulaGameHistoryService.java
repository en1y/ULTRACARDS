...
public class BriskulaGameHistoryService {
 ...
 @Transactional(readOnly=true) public List<ShortGameHistoryDTO> getPastGames(UserEntity user,int offset,String result,String sort){var games=("oldest".equalsIgnoreCase(sort)||"asc".equalsIgnoreCase(sort))?repository.findPastGamesByUserIdOldest(user.getId(),Math.max(0,offset)):repository.findPastGamesByUserIdLatest(user.getId(),Math.max(0,offset));var out=new ArrayList<ShortGameHistoryDTO>();for(var game:games){var dto=shortHistory(game);var win=contains(dto.getWinners(),user.getId());if(!"wins".equalsIgnoreCase(result)&&!"win".equalsIgnoreCase(result)||win)out.add(dto);else if("loss".equalsIgnoreCase(result)||"losses".equalsIgnoreCase(result)||"lose".equalsIgnoreCase(result))out.add(dto);}return out;}
