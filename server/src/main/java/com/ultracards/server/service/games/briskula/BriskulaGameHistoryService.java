...
public class BriskulaGameHistoryService {
 ...
 private void add(RecordedBriskulaGame g,Map<Long,Integer> p,RecordedPlayer w,int v){if(w==null)return;p.put(w.id(),p.getOrDefault(w.id(),0)+v);if(g.teamsEnabled()){var ids=g.teamUserIds();var i=ids.indexOf(w.id());if(ids.size()!=4||i<0)return;var mate=ids.get(i<2?1-i:5-i);p.put(mate,p.getOrDefault(mate,0)+v);}}
 ...
 private List<List<GamePlayerDTO>> teams(RecordedBriskulaGame g){var r=new ArrayList<List<GamePlayerDTO>>();if(!g.teamsEnabled())return r;var map=new HashMap<Long,GamePlayerDTO>();for(var x:g.players())map.put(x.id(),player(x));var ids=g.teamUserIds();if(ids.size()!=4||ids.stream().anyMatch(id->!map.containsKey(id)))return r;r.add(List.of(map.get(ids.get(0)),map.get(ids.get(1))));r.add(List.of(map.get(ids.get(2)),map.get(ids.get(3))));return r;}
